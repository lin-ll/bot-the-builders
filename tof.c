...
//
int tofInit(int bLongRange, int pi_arg, int handle)
{
char filename[32];
  pi = pi_arg;
  file_i2c = handle;
...
static unsigned short readReg16(unsigned char ucAddr)
{
  //adafruit_distance_read16((int)ucAddr);
...
static unsigned char readReg(unsigned char ucAddr)
{
  //adafruit_distance_read8((int)ucAddr);
  //return ucAddr;
...
int rc;
        ...
        ucTemp = ucAddr;
        rc = i2c_write_device(pi, file_i2c, &ucTemp, 1);
	if (rc == 1)
	{
        	rc = i2c_read_device(pi, file_i2c, &ucTemp, 1);
		if (rc != 1) {};
	}
	return ucTemp;
} /* ReadReg() */
...
int rc;
	...
	if (rc == 1)
	{
		rc = i2c_read_device(pi, file_i2c, pBuf, iCount);
		if (rc != iCount) {};
	}
} /* readMulti() */
...
int rc;
	...
	rc = i2c_write_device(pi, file_i2c, ucTemp, iCount+1);
	if (rc != iCount+1) {};
} /* writeMulti() */
//
...
int rc;
	...
	rc = i2c_write_device(pi, file_i2c, ucTemp, 3);
...
static void writeReg(unsigned char ucAddr, unsigned char ucValue)
{
  // adafruit_distance_write8((int)ucAddr, (int)ucValue);
unsigned char ucTemp[2];
int rc;

	ucTemp[0] = ucAddr;
	ucTemp[1] = ucValue;
	rc = i2c_write_device(pi, file_i2c, ucTemp, 2);
...
int rc;
	...
	while (ucCount)
	{
		rc = i2c_write_device(pi, file_i2c, ucList, 2);
		if (rc != 2) {};
		ucList += 2;
		ucCount--;
	}
...
int i;
	...
	if (model)
	{
		ucTemp[0] = REG_IDENTIFICATION_MODEL_ID;
        	i = i2c_write_device(pi, file_i2c, ucTemp, 1); // write address of register to read
        	i = i2c_read_device(pi, file_i2c, ucTemp, 1);
		if (i == 1)
			*model = ucTemp[0];
	}
	if (revision)
	{
		...
		i = i2c_write_device(pi, file_i2c, ucTemp, 1);
		i = i2c_read_device(pi, file_i2c, ucTemp, 1);
		if (i == 1)
			*revision = ucTemp[0];
	}
